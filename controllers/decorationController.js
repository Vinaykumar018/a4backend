  ...
  async (req, res) => {
    try {
      const {
        ...
        isOffer,
        status,
        child_categories = null // ✅ Default to null if not provided
      } = req.body;
      ...
      const newProduct = new Product({
        ...
        isOffer,
        status,
        child_categories: JSON.parse(child_categories || 'null') // ✅ Ensure parsed if sent as JSON string
      });
      ...
      res.status(200).json({ message: 'Product created', status: 1, data: newProduct });
    } catch (err) {
